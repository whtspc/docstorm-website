...
export class DemoContainer extends HTMLElement {
  ...
  _getStyles() {
    return `
      ...
      :host {
        display: block;
        font-family: 'YAF6P3TkODE_0', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        background: #fff;
      }
      ...
      /* Footer section with Request a Demo button */
      .demo-footer {
        text-align: center;
        padding: 24px;
      }
      ...
      .demo-footer__btn:hover {
        background: rgb(60, 90, 170);
        transform: translateY(-2px);
      }

      .demo-footer__btn:active {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="demo-header">
        <h2 class="demo-header__title">Try it out yourself</h2>
      </div>
  ...
  _setupEventListeners() {
    ...
    // File drag start - also update preview to show the dragged file
    this.shadowRoot.addEventListener('file-drag-start', (e) => {
      this._draggingFile = e.detail.file;
      this._state = 'dragging';
      this._updateCaption();

      // Update preview to show the file being dragged
      const filePreview = this.shadowRoot.querySelector('docstorm-file-preview');
      if (filePreview) {
        filePreview.file = e.detail.file;
      }
    });
